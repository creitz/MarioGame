...
namespace Sprint0Game
{
    public class Level : ILevel
    {
        ...
        public MarioPowerLevel OriginalPowerLevel { get; set; }
        ...
        {
            string file = string.Format("Content/{0}.txt", this.LevelLoadString);
            LevelCreator creator = new LevelCreator(this);
            this.Background = new Background();
            creator.GenerateLevel(file);
            if (OriginalPowerLevel == MarioPowerLevel.Fire || OriginalPowerLevel == MarioPowerLevel.Metal)
                this.Mario.RespondToRequest(MarioActionRequest.GoFire);
            else if (OriginalPowerLevel == MarioPowerLevel.Big)
